...
func TestNotify(t *testing.T) {
	...
	tenantID := uint64(1)
	s.Set(storage.QueueMetaKey(tenantID, 0), protoc.MustMarshal(&metapb.QueueState{
		Partitions: 2,
	}))
	...
	data, err := s.Get(storage.QueueItemKey(storage.PartitionKey(tenantID, 0), 1))
...
func TestNotifyWithConditionNotExist(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		...
		data, err = s.Get(storage.QueueItemKey(storage.PartitionKey(tenantID, 0), 2))
		assert.NoError(t, err, "TestNotifyWithConditionNotExist failed")
...
func TestNotifyWithConditionEqual(t *testing.T) {
	...
	for i := 0; i < 1; i++ {
		...
		keys, _, err := s.Scan(storage.QueueItemKey(storage.PartitionKey(tenantID, 0), 1), storage.QueueItemKey(storage.PartitionKey(tenantID, 0), 100), 100)
		assert.NoError(t, err, "TestNotifyWithConditionEqual failed")
